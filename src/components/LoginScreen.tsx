...
export const LoginScreen: React.FC<LoginScreenProps> = ({ asModal = false, onClose }) => {
  ...
  if (asModal) {
    return (
      ...
      >
        <div className="glass-card w-full max-w-md rounded-3xl p-8 border border-slate-800 shadow-2xl relative z-10 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
          {/* Close button */}
          {onClose && (
            <button
              onClick={onClose}
          ...
          <LoginFormContent
            ...
            handleQuickFillAdmin={handleQuickFillAdmin}
          />
        </div>
      </div>
    );
...
interface FormProps {
  ...
  handleQuickFillAdmin: () => void;
}

const LoginFormContent: React.FC<FormProps> = ({
  username, setUsername, password, setPassword,
  showPassword, setShowPassword, errorMsg,
  handleLoginSubmit, handleQuickFillAdmin,
}) => (
